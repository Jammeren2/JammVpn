...
const RELEASE_NOTES = [
  "ГЛАВНОЕ: WinDivert больше не кладёт интернет — устранён блэкхол всего трафика при сбое/смене драйвера.",
  "Раздельный туннель устойчивее: паника/сбой драйвера не роняет связь; меньше утечек памяти.",
  "Надёжнее UDP (VRChat, голос) — без затыков при всплеске соединений; IPv6 split-приложений больше не уходит в никуда.",
  "Статистика: разделение TCP/UDP (бейдж у соединения + сводка).",
  "Десятки исправлений по итогам аудита кода (атрибуция, утечки сокетов, бэкпрешер, жизненный цикл контроллера, парсеры).",
  "Раздельный туннель надёжнее на ПК с несколькими сетевыми адаптерами (оба драйвера).",
  "Подписки: ноды объединяются по балансировщикам, кнопка «★ лучший» выбирает быстрейшую; флаг страны рисуется картинкой.",
  "Статистика: виден процесс-инициатор соединения.",
  "Тест соединения показывает реальную причину, если узел не поднимается.",
];
